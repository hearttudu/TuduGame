...
public class AI implements Runnable {
    ...
    //电脑执子（默认黑子）
    private int aiChess = FiveChessAiView.BLACK_CHESS;
    //所有无子位置的信息集合
    ...
    private int checkUser(int x, int y) {
        int userChess;
        if (aiChess == FiveChessAiView.WHITE_CHESS) {
            userChess = FiveChessAiView.BLACK_CHESS;
        } else {
            userChess = FiveChessAiView.WHITE_CHESS;
        }
    ...
    public void run() {
        ...
        for (int i = 0; i < panelLength; i++)
            for (int j = 0; j < panelLength; j++) {
                if (chessArray[i][j] == FiveChessAiView.NO_CHESS) {
    ...
    private int getHorizontalPriority(int x, int y, int chess) {
        ...
        if (y == 0) {
            ...
            for (int i = y - 1; i >= 0; i--) {
                ...
                if (chessArray[x][i] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isStartStem = chessArray[x][i] != FiveChessAiView.NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == 0) {
        ...
        if (y == panelLength - 1) {
            ...
            for (int i = y + 1; i < panelLength; i++) {
                ...
                if (chessArray[x][i] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isEndStem = chessArray[x][i] != FiveChessAiView.NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == panelLength - 1) {
    ...
    private int getVerticalPriority(int x, int y, int chess) {
        ...
        if (x == 0) {
            ...
            for (int i = x - 1; i >= 0; i--) {
                ...
                if (chessArray[i][y] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isStartStem = chessArray[i][y] != FiveChessAiView.NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == 0) {
        ...
        if (x == panelLength - 1) {
            ...
            for (int i = x + 1; i < panelLength; i++) {
                ...
                if (chessArray[i][y] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isEndStem = chessArray[i][y] != FiveChessAiView.NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == panelLength - 1) {
    ...
    private int getLeftSlashPriority(int x, int y, int chess) {
        ...
        if (x == 0 || y == 0) {
            ...
            for (int i = x - 1, j = y - 1; i >= 0 && j >= 0; i--, j--) {
                ...
                if (chessArray[i][j] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isStartStem = chessArray[i][j] != FiveChessAiView.NO_CHESS;
                    break;
                } else {
                    connectCount++;
                    if (i == 0 || j == 0) {
        ...
        if (x == panelLength - 1 || y == panelLength - 1) {
            ...
            for (int i = x + 1, j = y + 1; i < panelLength && j < panelLength; i++, j++) {
                ...
                if (chessArray[i][j] != chess) {
                    //不是自己的棋子，则左边被堵住或者是空位
                    isEndStem = chessArray[i][j] != FiveChessAiView.NO_CHESS;
                    break;
                } else {
                    connectCount++;
